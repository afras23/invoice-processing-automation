...
def test_all_fields_present_but_validation_failed():
    extracted = ExtractedInvoice(
        vendor="Acme", invoice_id="INV-1", date="2026-03-01", amount=-1.0
    )
    result = score_confidence(extracted, _failed(["amount must be greater than zero"]))
    assert result.completeness == 1.0
    assert result.validation_score == 0.0
    assert result.score == pytest_approx(0.6)
...
def test_half_fields_present_and_valid():
    ...
    assert result.validation_score == 1.0
    assert result.score == pytest_approx(0.7)
...
# Use a simple approx helper to avoid adding pytest dependency just for this
def pytest_approx(value: float, rel: float = 1e-4) -> "_Approx":
